  ...
  <div>
    ...
    <div class="gva-table-box">
      <div class="gva-btn-list">
        <el-button type="primary" icon="Present" @click="openDrawDialog"
          >随机抽奖</el-button
        >
    ...
    >
      ...
      >
        <el-form-item label="选择活动" prop="activityId">
          <el-select
            v-model="drawForm.activityId"
            placeholder="请选择活动"
            style="width: 100%"
            @change="handleDrawActivityChange"
          >
            <el-option
              v-for="item in activityList"
              :key="item.ID"
      ...
      <template #footer>
        ...
        <el-button type="primary" :loading="drawLoading" @click="handleDraw"
          >开始抽奖</el-button
        >
      </template>
    </el-dialog>
    ...
    <el-dialog v-model="resultVisible" title="🎉 中奖名单" width="500px">
      <div class="winner-list">
        <div
          v-for="(item, index) in drawResult"
          :key="index"
          class="winner-item"
        >
          ...
          <div class="winner-info">
            ...
            <div class="winner-real-name">
              {{ item.realName }} - {{ item.department }}
            </div>
          </div>
        </div>
      </div>
...
<script setup>
  ...
  const handleDelete = async (row) => {
    await ElMessageBox.confirm('确定要删除该中奖记录吗？', '提示', {
      type: 'warning'
    })
